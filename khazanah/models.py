...
class Transaksi(models.Model):
    khazanah = models.ForeignKey(Khazanah, on_delete=models.CASCADE)
    nomor_inventaris = models.CharField(max_length=50)
    lokasi_penggunaan = models.CharField(max_length=200,default='-')
    ...
    waktu_produksi_kertas = models.DateField(blank=True, null=True)
    kondisi_fisik_kertas = models.ForeignKey(KondisiFisikKertas, on_delete=models.CASCADE)
    panjang_kertas = models.DecimalField(max_digits=10, decimal_places=2,verbose_name='Panjang Kertas (mm)')
    lebar_kertas = models.DecimalField(max_digits=10, decimal_places=2,verbose_name='Lebar Kertas (mm)')
    ketebalan_kertas = models.DecimalField(max_digits=10, decimal_places=2,default='',verbose_name='Tebal Kertas (mm)')
    warna_kertas = models.ForeignKey(WarnaKertas, on_delete=models.CASCADE)
    gramatur_kertas = models.DecimalField(max_digits=10, decimal_places=2,verbose_name='Gramatur Kertas (gr)')
    tingkat_keasaman = models.CharField(max_length=50, blank=True, null=True)
    jarak_antara_kolom_garis = models.DecimalField(max_digits=10, decimal_places=2)
    motif_watermark = models.ForeignKey(MotifWatermark, on_delete=models.CASCADE)
    posisi_watermark = models.ForeignKey(PosisiWatermark, on_delete=models.CASCADE)
    panjang_watermark = models.DecimalField(max_digits=10, decimal_places=2,verbose_name='Panjang Watermark (mm)')
    lebar_watermark = models.DecimalField(max_digits=10, decimal_places=2,verbose_name='Lebar Watermark (mm)')
    motif_countermark = models.ForeignKey(MotifCountermark, on_delete=models.CASCADE)
    posisi_countermark = models.ForeignKey(PosisiCountermark, on_delete=models.CASCADE)
    panjang_countermark = models.DecimalField(max_digits=10, decimal_places=2,verbose_name='Panjang Countermark (mm)')
    lebar_countermark = models.DecimalField(max_digits=10, decimal_places=2,verbose_name='Lebar Countermark (mm)')
    pabrik_kertas = models.ForeignKey(Pabrik, on_delete=models.CASCADE)
